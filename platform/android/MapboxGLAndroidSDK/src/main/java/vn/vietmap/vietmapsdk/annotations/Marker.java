 ...
 * {@link LatLng} and using {@link VietmapMap#addMarker(MarkerOptions)}. The marker icon will be
...
public class Marker extends Annotation {
  ...
  private void refreshInfoWindowContent() {
    if (isInfoWindowShown() && mapView != null && vietmapMap != null && vietmapMap.getInfoWindowAdapter() == null) {
      InfoWindow infoWindow = getInfoWindow(mapView);
      if (mapView.getContext() != null) {
        infoWindow.adaptDefaultMarker(this, vietmapMap, mapView);
      }
      VietmapMap map = getVietmapMap();
      if (map != null) {
        map.updateMarker(this);
  ...
  /**
   * Do not use this method, used internally by the SDK. Use {@link VietmapMap#selectMarker(Marker)}
   * if you want to programmatically display the markers info window.
   *
   * @param vietmapMap The hosting mapbox map.
   * @param mapView   The hosting map view.
   * @return The info window that was shown.
   */
  @Nullable
  public InfoWindow showInfoWindow(@NonNull VietmapMap vietmapMap, @NonNull MapView mapView) {
    setVietmapMap(vietmapMap);
    setMapView(mapView);
    VietmapMap.InfoWindowAdapter infoWindowAdapter = getVietmapMap().getInfoWindowAdapter();
    if (infoWindowAdapter != null) {
      // end developer is using a custom InfoWindowAdapter
      View content = infoWindowAdapter.getInfoWindow(this);
      if (content != null) {
        infoWindow = new InfoWindow(content, vietmapMap);
  ...
  private InfoWindow getInfoWindow(@NonNull MapView mapView) {
    if (infoWindow == null && mapView.getContext() != null) {
      infoWindow = new InfoWindow(mapView, R.layout.mapbox_infowindow_content, getVietmapMap());
    }
    return infoWindow;
  }
