...
import vn.vietmap.vietmapsdk.utils.ThreadUtils;
...
@Keep
public final class Vietmap {

  private static final String TAG = "Mbgl-Vietmap";
  private static ModuleProvider moduleProvider;
  private static Vietmap INSTANCE;
   ...
   * @return the single instance of Vietmap
   */
  @UiThread
  @NonNull
  public static synchronized Vietmap getInstance(@NonNull Context context) {
    ThreadUtils.init(context);
    ThreadUtils.checkThread(TAG);
    if (INSTANCE == null) {
      Context appContext = context.getApplicationContext();
      FileSource.initializeFileDirsPaths(appContext);
      INSTANCE = new Vietmap(appContext, null);
  ...
  @UiThread
  @NonNull
  public static synchronized Vietmap getInstance(@NonNull Context context, @Nullable String apiKey,
    ...
    if (INSTANCE == null) {
      Context appContext = context.getApplicationContext();
      FileSource.initializeFileDirsPaths(appContext);
      INSTANCE = new Vietmap(appContext, apiKey);
      ConnectivityReceiver.instance(appContext);
    } else {
  ...
  Vietmap(@NonNull Context context, @Nullable String apiKey) {
    this.context = context;
    this.apiKey = apiKey;
  }

  Vietmap(@NonNull Context context, @Nullable String apiKey, @NonNull TileServerOptions options) {
    this.context = context;
    this.apiKey = apiKey;
  ...
  /**
   * Runtime validation of Vietmap creation.
   */
  private static void validateVietmap() {
    if (INSTANCE == null) {
  ...
  /**
   * Runtime validation of Vietmap access token
   *
  ...
  static boolean isApiKeyValid(@Nullable String apiKey) {
    ...
    apiKey = apiKey.trim().toLowerCase(VietmapConstants.MAPBOX_LOCALE);
  ...
  /**
   * Internal use. Check if the {@link Vietmap#INSTANCE} is present.
   */
