...
public class MapView extends FrameLayout implements NativeMapView.ViewCallback {
  ...
  VietMapOptions VietmapMapOptions;
  ...
  public MapView(@NonNull Context context) {
    super(context);
    initialize(context, VietMapOptions.createFromAttributes(context));
  }

  @UiThread
  public MapView(@NonNull Context context, @Nullable AttributeSet attrs) {
    super(context, attrs);
    initialize(context, VietMapOptions.createFromAttributes(context, attrs));
  }
  ...
  @UiThread
  public MapView(@NonNull Context context, @Nullable VietMapOptions options) {
    super(context);
    initialize(context, options == null ? VietMapOptions.createFromAttributes(context) : options);
  }

  @CallSuper
  @UiThread
  protected void initialize(@NonNull final Context context, @NonNull final VietMapOptions options) {
    if (isInEditMode()) {
      // in IDE layout editor, just return
      return;
    }

    if (!Vietmap.hasInstance()) {
      throw new VietmapConfigurationException();
    }

    // hide surface until map is fully loaded #10990
    setForeground(new ColorDrawable(options.getForegroundLoadColor()));

    VietmapMapOptions = options;
  ...
  private void initialiseMap() {
    ...
    // setup components for VietmapMap creation
    ...
    // VietmapMap
    List<VietmapMap.OnDeveloperAnimationListener> developerAnimationListeners = new ArrayList<>();
    VietmapMap = new VietmapMap(nativeMapView, transform, uiSettings, proj, registerTouchListener, cameraDispatcher,
      developerAnimationListeners);
    VietmapMap.injectAnnotationManager(annotationManager);
    ...
    // LocationComponent
    VietmapMap.injectLocationComponent(new LocationComponent(VietmapMap, transform, developerAnimationListeners));
    ...
    nativeMapView.setReachability(Vietmap.isConnected());

    // initialise VietmapMap
    if (savedInstanceState == null) {
      VietmapMap.initialise(context, VietmapMapOptions);
    } else {
      VietmapMap.onRestoreInstanceState(savedInstanceState);
    }
  ...
  protected ImageView initialiseAttributionView() {
    ...
    // inject widgets with VietmapMap
    attrView.setOnClickListener(attributionClickListener = new AttributionClickListener(getContext(), VietmapMap));
    return attrView;
  }
  ...
  private VietmapMap.OnCompassAnimationListener createCompassAnimationListener(@NonNull final CameraChangeDispatcher
                                                                                cameraChangeDispatcher) {
    return new VietmapMap.OnCompassAnimationListener() {
      @Override
  ...
  private OnClickListener createCompassClickListener(@NonNull final CameraChangeDispatcher cameraChangeDispatcher) {
    return new OnClickListener() {
      ...
      public void onClick(View v) {
        if (VietmapMap != null && compassView != null) {
          if (focalPoint != null) {
            VietmapMap.setFocalBearing(0, focalPoint.x, focalPoint.y, CompassView.TIME_MAP_NORTH_ANIMATION);
          } else {
            VietmapMap.setFocalBearing(0, VietmapMap.getWidth() / 2, VietmapMap.getHeight() / 2, CompassView.TIME_MAP_NORTH_ANIMATION);
          }
          cameraChangeDispatcher.onCameraMoveStarted(vn.vietmap.vietmapsdk.maps.VietmapMap.OnCameraMoveStartedListener.REASON_API_ANIMATION);
...
//   * </p>
//   * You must set a valid access token with {@link Vietmap#getInstance(Context, String)}
...
//   */
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    if (savedInstanceState != null && savedInstanceState.getBoolean(VietmapConstants.STATE_HAS_SAVED_STATE)) {
      this.savedInstanceState = savedInstanceState;
    }
  }

  private void initialiseDrawingSurface(VietMapOptions options) {
    ...
    if (options.getTextureMode()) {
      ...
      VietmapGLSurfaceView glSurfaceView = new VietmapGLSurfaceView(getContext());
      glSurfaceView.setZOrderMediaOverlay(VietmapMapOptions.getRenderSurfaceOnTop());
    ...
    boolean crossSourceCollisions = VietmapMapOptions.getCrossSourceCollisions();
  ...
  public void onSaveInstanceState(@NonNull Bundle outState) {
    if (VietmapMap != null) {
      outState.putBoolean(VietmapConstants.STATE_HAS_SAVED_STATE, true);
      VietmapMap.onSaveInstanceState(outState);
    }
  }
  ...
  public void onLowMemory() {
    if (nativeMapView != null && VietmapMap != null && !destroyed) {
   ...
   * Can be overwritten using {@link VietMapOptions#pixelRatio(float)}.
   *
   * @return the current map pixel ratio
   */
  public float getPixelRatio() {
    // check is user defined his own pixel ratio value
    float pixelRatio = VietmapMapOptions.getPixelRatio();
    if (pixelRatio == 0) {
   ...
   * <p>
   * Calling {@link VietmapMap#snapshot(VietmapMap.SnapshotReadyCallback)} from this callback
  ...
  /**
   * Sets a callback object which will be triggered when the {@link VietmapMap} instance is ready to be used.
   *
  ...
  public void getMapAsync(final @NonNull OnMapReadyCallback callback) {
    if (VietmapMap == null) {
      ...
      callback.onMapReady(VietmapMap);
    }
  }
  ...
  @Nullable
  VietmapMap getVietmapMap() {
    return VietmapMap;
  }

  void setVietmapMap(VietmapMap VietmapMap) {
  ...
  private class InitialRenderCallback implements OnDidFinishRenderingFrameListener {
    ...
    public void onDidFinishRenderingFrame(boolean fully) {
      if (VietmapMap != null && VietmapMap.getStyle() != null && VietmapMap.getStyle().isFullyLoaded()) {
        renderCount++;
        if (renderCount == 3) {
  ...
  private class GesturesManagerInteractionListener implements VietmapMap.OnGesturesManagerInteractionListener {

    @Override
    public void onAddMapClickListener(VietmapMap.OnMapClickListener listener) {
      mapGestureDetector.addOnMapClickListener(listener);
    }

    @Override
    public void onRemoveMapClickListener(VietmapMap.OnMapClickListener listener) {
      mapGestureDetector.removeOnMapClickListener(listener);
    }

    @Override
    public void onAddMapLongClickListener(VietmapMap.OnMapLongClickListener listener) {
      mapGestureDetector.addOnMapLongClickListener(listener);
    }

    @Override
    public void onRemoveMapLongClickListener(VietmapMap.OnMapLongClickListener listener) {
      mapGestureDetector.removeOnMapLongClickListener(listener);
    }

    @Override
    public void onAddFlingListener(VietmapMap.OnFlingListener listener) {
      mapGestureDetector.addOnFlingListener(listener);
    }

    @Override
    public void onRemoveFlingListener(VietmapMap.OnFlingListener listener) {
      mapGestureDetector.removeOnFlingListener(listener);
    }

    @Override
    public void onAddMoveListener(VietmapMap.OnMoveListener listener) {
      mapGestureDetector.addOnMoveListener(listener);
    }

    @Override
    public void onRemoveMoveListener(VietmapMap.OnMoveListener listener) {
      mapGestureDetector.removeOnMoveListener(listener);
    }

    @Override
    public void onAddRotateListener(VietmapMap.OnRotateListener listener) {
      mapGestureDetector.addOnRotateListener(listener);
    }

    @Override
    public void onRemoveRotateListener(VietmapMap.OnRotateListener listener) {
      mapGestureDetector.removeOnRotateListener(listener);
    }

    @Override
    public void onAddScaleListener(VietmapMap.OnScaleListener listener) {
      mapGestureDetector.addOnScaleListener(listener);
    }

    @Override
    public void onRemoveScaleListener(VietmapMap.OnScaleListener listener) {
      mapGestureDetector.removeOnScaleListener(listener);
    }

    @Override
    public void onAddShoveListener(VietmapMap.OnShoveListener listener) {
      mapGestureDetector.addShoveListener(listener);
    }

    @Override
    public void onRemoveShoveListener(VietmapMap.OnShoveListener listener) {
  ...
  private class MapCallback implements OnDidFinishLoadingStyleListener,
    ...
    void initialised() {
      VietmapMap.onPreMapReady();
      onMapReady();
      VietmapMap.onPostMapReady();
    }
    ...
    public void onDidFinishRenderingFrame(boolean fully) {
      if (VietmapMap != null) {
        VietmapMap.onUpdateFullyRendered();
      }
    }
    ...
    public void onDidFinishLoadingMap() {
      if (VietmapMap != null) {
        VietmapMap.onUpdateRegionChange();
      }
    }
  ...
  private static class AttributionClickListener implements OnClickListener {
    ...
    private AttributionClickListener(@NonNull Context context, @NonNull VietmapMap mapboxMap) {
