...
class MarkerContainer implements Markers {
  ...
  @Override
  public Marker addBy(@NonNull BaseMarkerOptions markerOptions, @NonNull VietmapMap VietmapMap) {
    Marker marker = prepareMarker(markerOptions);
    long id = nativeMapView != null ? nativeMapView.addMarker(marker) : 0;
    marker.setVietmapMap(VietmapMap);
    marker.setId(id);
  ...
  @NonNull
  @Override
  public List<Marker> addBy(@NonNull List<? extends BaseMarkerOptions> markerOptionsList, @NonNull VietmapMap
    VietmapMap) {
    ...
    if (nativeMapView != null && count > 0) {
      ...
      if (markers.size() > 0) {
        ...
        for (int i = 0; i < ids.length; i++) {
          ...
          createdMarker.setVietmapMap(VietmapMap);
  ...
  @Override
  public void update(@NonNull Marker updatedMarker, @NonNull VietmapMap VietmapMap) {
    ensureIconLoaded(updatedMarker, VietmapMap);
  ...
  private void ensureIconLoaded(Marker marker, @NonNull VietmapMap VietmapMap) {
    iconManager.ensureIconLoaded(marker, VietmapMap);
  }
