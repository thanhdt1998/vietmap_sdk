...
final class LocationLayerController {
  ...
  LocationLayerController(VietmapMap VietmapMap, Style style,
  ...
  private void styleScaling(@NonNull LocationComponentOptions options) {
    Expression scaleExpression = interpolate(linear(), zoom(),
      stop(VietmapMap.getMinZoomLevel(), options.minZoomIconScale()),
      stop(VietmapMap.getMaxZoomLevel(), options.maxZoomIconScale())
    );
  ...
  boolean onMapClick(@NonNull LatLng point) {
    PointF screenLoc = VietmapMap.getProjection().toScreenLocation(point);
    List<Feature> features = VietmapMap.queryRenderedFeatures(screenLoc,
