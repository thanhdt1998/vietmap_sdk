...
import static vn.vietmap.vietmapsdk.location.LocationComponentConstants.ACCURACY_RADIUS_ANIMATION_DURATION;
import static vn.vietmap.vietmapsdk.location.LocationComponentConstants.COMPASS_UPDATE_RATE_MS;
import static vn.vietmap.vietmapsdk.location.LocationComponentConstants.MAX_ANIMATION_DURATION_MS;
...
import static vn.vietmap.vietmapsdk.location.MapboxAnimator.ANIMATOR_PULSING_CIRCLE;
...
import static vn.vietmap.vietmapsdk.location.Utils.immediateAnimation;
import static vn.vietmap.vietmapsdk.location.Utils.normalize;
import static vn.vietmap.vietmapsdk.location.Utils.shortestRotation;
...
final class LocationAnimatorCoordinator {
  ...
  void feedNewLocation(@NonNull @Size(min = 1) Location[] newLocations,
    ...
    if (previousLocation == null) {
      ...
      locationUpdateTimestamp = SystemClock.elapsedRealtime() - LocationComponentConstants.TRANSITION_ANIMATION_DURATION_MS;
    }
    ...
    if (!snap) {
      ...
      animationDuration = Math.min(animationDuration, LocationComponentConstants.MAX_ANIMATION_DURATION_MS);
    }
  ...
  void feedNewCompassBearing(float targetCompassBearing, @NonNull CameraPosition currentCameraPosition) {
    ...
    playAnimators(
      compassAnimationEnabled ? LocationComponentConstants.COMPASS_UPDATE_RATE_MS : 0,
  ...
  void feedNewAccuracyRadius(float targetAccuracyRadius, boolean noAnimation) {
    ...
    playAnimators(
      noAnimation || !accuracyAnimationEnabled ? 0 : LocationComponentConstants.ACCURACY_RADIUS_ANIMATION_DURATION,
  ...
  void resetAllCameraAnimations(@NonNull CameraPosition currentCameraPosition, boolean isGpsNorth) {
    ...
    playAnimators(
      snap ? 0 : LocationComponentConstants.TRANSITION_ANIMATION_DURATION_MS,
      MapboxAnimator.ANIMATOR_CAMERA_LATLNG,
      MapboxAnimator.ANIMATOR_CAMERA_GPS_BEARING);
  }
