...
class IconManager {
  ...
  private Icon loadDefaultIconForMarker(Marker marker) {
    Icon icon = IconFactory.getInstance(Vietmap.getApplicationContext()).defaultMarker();
  ...
  void ensureIconLoaded(@NonNull Marker marker, @NonNull VietmapMap VietmapMap) {
    Icon icon = marker.getIcon();
    if (icon == null) {
      icon = loadDefaultIconForMarker(marker);
    }
    addIcon(icon);
    setTopOffsetPixels(marker, VietmapMap, icon);
  }

  private void setTopOffsetPixels(Marker marker, @NonNull VietmapMap VietmapMap, @NonNull Icon icon) {
    // this seems to be a costly operation according to the profiler so I'm trying to save some calls
    Marker previousMarker = marker.getId() != -1 ? (Marker) VietmapMap.getAnnotation(marker.getId()) : null;
