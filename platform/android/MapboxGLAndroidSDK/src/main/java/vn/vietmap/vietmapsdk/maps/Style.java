 ...
 * VietmapMap. This object is returned from {@link VietmapMap#getStyle()} once the style
...
//   */
    ...
    if (!fullyLoaded) {
      ...
      for (Builder.LayerWrapper layerWrapper : builder.layers) {
        if (layerWrapper instanceof Builder.LayerAtWrapper) {
          ...
          addLayerBelow(layerWrapper.layer, VietmapConstants.LAYER_ID_ANNOTATIONS);
        }
      }
  ...
  public static class Builder {
     ...
     * callback or provide an {@link OnStyleLoaded} callback when setting the style on VietmapMap.
     * </p>
     ...
     * This method is asynchronous and will return before the style finishes loading.
     * If you wish to wait for the map to finish loading, listen to the {@link MapView.OnDidFinishLoadingStyleListener}
     * callback or use {@link VietmapMap#setStyle(String, OnStyleLoaded)} instead.
     * </p>
  ...
  public static DefaultStyle[] getPredefinedStyles() {
    return Vietmap.getPredefinedStyles();
  }
  ...
  public static String getPredefinedStyle(String name) {
    DefaultStyle style = Vietmap.getPredefinedStyle(name);
    if (style != null) {
      return style.getUrl();
    }
