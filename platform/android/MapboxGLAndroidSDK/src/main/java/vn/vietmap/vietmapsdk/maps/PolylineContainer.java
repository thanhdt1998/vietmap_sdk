...
class PolylineContainer implements Polylines {
  ...
  @Override
  public Polyline addBy(@NonNull PolylineOptions polylineOptions, @NonNull VietmapMap VietmapMap) {
    Polyline polyline = polylineOptions.getPolyline();
    if (!polyline.getPoints().isEmpty()) {
      long id = nativeMap != null ? nativeMap.addPolyline(polyline) : 0;
      polyline.setVietmapMap(VietmapMap);
      polyline.setId(id);
  ...
  @NonNull
  @Override
  public List<Polyline> addBy(@NonNull List<PolylineOptions> polylineOptionsList, @NonNull VietmapMap VietmapMap) {
    ...
    if (nativeMap != null && count > 0) {
      ...
      for (int i = 0; i < ids.length; i++) {
        ...
        polylineCreated.setVietmapMap(VietmapMap);
