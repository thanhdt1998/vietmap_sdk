...
import static vn.vietmap.vietmapsdk.constants.VietmapConstants.MAXIMUM_ANGULAR_VELOCITY;
import static vn.vietmap.vietmapsdk.constants.VietmapConstants.MAX_ABSOLUTE_SCALE_VELOCITY_CHANGE;
import static vn.vietmap.vietmapsdk.constants.VietmapConstants.QUICK_ZOOM_MAX_ZOOM_CHANGE;
import static vn.vietmap.vietmapsdk.constants.VietmapConstants.ROTATE_VELOCITY_RATIO_THRESHOLD;
import static vn.vietmap.vietmapsdk.constants.VietmapConstants.SCALE_VELOCITY_ANIMATION_DURATION_MULTIPLIER;
import static vn.vietmap.vietmapsdk.constants.VietmapConstants.SCALE_VELOCITY_RATIO_THRESHOLD;
import static vn.vietmap.vietmapsdk.constants.VietmapConstants.ZOOM_RATE;
import static vn.vietmap.vietmapsdk.maps.VietmapMap.OnCameraMoveStartedListener.REASON_API_GESTURE;
import static vn.vietmap.vietmapsdk.utils.MathUtils.normalize;
...
final class MapGestureDetector {
  ...
  // new map touch API
  private final CopyOnWriteArrayList<VietmapMap.OnMapClickListener> onMapClickListenerList
    = new CopyOnWriteArrayList<>();

  private final CopyOnWriteArrayList<VietmapMap.OnMapLongClickListener> onMapLongClickListenerList
    = new CopyOnWriteArrayList<>();

  private final CopyOnWriteArrayList<VietmapMap.OnFlingListener> onFlingListenerList
    = new CopyOnWriteArrayList<>();

  private final CopyOnWriteArrayList<VietmapMap.OnMoveListener> onMoveListenerList
    = new CopyOnWriteArrayList<>();

  private final CopyOnWriteArrayList<VietmapMap.OnRotateListener> onRotateListenerList
    = new CopyOnWriteArrayList<>();

  private final CopyOnWriteArrayList<VietmapMap.OnScaleListener> onScaleListenerList
    = new CopyOnWriteArrayList<>();

  private final CopyOnWriteArrayList<VietmapMap.OnShoveListener> onShoveListenerList
    = new CopyOnWriteArrayList<>();
   ...
   * {@link VietmapConstants#SCHEDULED_ANIMATION_TIMEOUT}
   */
  @NonNull
  ...
  private void scheduleAnimator(Animator animator) {
    ...
    animationsTimeoutHandler.removeCallbacksAndMessages(null);
    animationsTimeoutHandler.postDelayed(cancelAnimatorsRunnable, VietmapConstants.SCHEDULED_ANIMATION_TIMEOUT);
  }
  ...
  private final class StandardGestureListener extends StandardGestureDetector.SimpleStandardOnGestureListener {
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
      ...
      if (velocityXY < VietmapConstants.VELOCITY_THRESHOLD_IGNORE_FLING) {
      ...
      long animationTime = (long) (velocityXY / 7 / tiltFactor + VietmapConstants.ANIMATION_DURATION_FLING_BASE);
      if (!uiSettings.isHorizontalScrollGesturesEnabled()) {
        ...
        if (angle > VietmapConstants.ANGLE_THRESHOLD_IGNORE_VERTICAL_FLING) {
          return false;
        }
        offsetX = 0.0;
      }
  ...
  private final class ScaleGestureListener extends StandardScaleGestureDetector.SimpleStandardOnScaleGestureListener {
    ...
    ScaleGestureListener(double densityMultiplier, float minimumGestureSpeed, float minimumAngledGestureSpeed,
      ...
      this.scaleVelocityRatioThreshold = VietmapConstants.SCALE_VELOCITY_RATIO_THRESHOLD * densityMultiplier;
    }
    ...
    public boolean onScale(@NonNull StandardScaleGestureDetector detector) {
      ...
      if (quickZoom) {
        ...
        double normalizedDeltaChange = MathUtils.normalize(pixelDeltaChange, 0, screenHeight, 0, VietmapConstants.QUICK_ZOOM_MAX_ZOOM_CHANGE);
        ...
        double zoomBy =
          (Math.log(detector.getScaleFactor()) / Math.log(Math.PI / 2)) * VietmapConstants.ZOOM_RATE * uiSettings.getZoomRate();
    ...
    public void onScaleEnd(@NonNull StandardScaleGestureDetector detector, float velocityX, float velocityY) {
      ...
      PointF focalPoint = getScaleFocalPoint(detector);
      // (log(x + 1 / e^2) + 2) * 150, x=0 to 2.5 (VietmapConstants#MAX_ABSOLUTE_SCALE_VELOCITY_CHANGE)
      long animationTime = (long) ((Math.log((Math.abs(zoomAddition)) + 1 / Math.pow(Math.E, 2)) + 2)
        * VietmapConstants.SCALE_VELOCITY_ANIMATION_DURATION_MULTIPLIER);
    ...
    private double calculateScale(double velocityXY, boolean isScalingOut) {
      double zoomAddition = velocityXY * VietmapConstants.MAX_ABSOLUTE_SCALE_VELOCITY_CHANGE * 1e-4;
      zoomAddition = MathUtils.clamp(zoomAddition, 0, VietmapConstants.MAX_ABSOLUTE_SCALE_VELOCITY_CHANGE);
      if (isScalingOut) {
  ...
  private final class RotateGestureListener extends RotateGestureDetector.SimpleOnRotateGestureListener {
    ...
    RotateGestureListener(float minimumScaleSpanWhenRotating, double densityMultiplier, float angularVelocityMultiplier,
      ...
      this.rotateVelocityRatioThreshold = VietmapConstants.ROTATE_VELOCITY_RATIO_THRESHOLD * densityMultiplier;
    ...
    public void onRotateEnd(@NonNull RotateGestureDetector detector, float velocityX,
      ...
      angularVelocity = MathUtils.clamp(angularVelocity, -VietmapConstants.MAXIMUM_ANGULAR_VELOCITY, VietmapConstants.MAXIMUM_ANGULAR_VELOCITY);
  ...
  private final class ShoveGestureListener extends ShoveGestureDetector.SimpleOnShoveGestureListener {
    ...
    public boolean onShove(@NonNull ShoveGestureDetector detector,
      ...
      pitch -= VietmapConstants.SHOVE_PIXEL_CHANGE_FACTOR * deltaPixelsSinceLast;
      pitch = MathUtils.clamp(pitch, VietmapConstants.MINIMUM_TILT, VietmapConstants.MAXIMUM_TILT);
  ...
  private void zoomAnimated(boolean zoomIn, @NonNull PointF zoomFocalPoint, boolean runImmediately) {
    ...
    scaleAnimator = createScaleAnimator(
      ...
      zoomIn ? 1 : -1,
      zoomFocalPoint,
      VietmapConstants.ANIMATION_DURATION);
    if (runImmediately) {
      scaleAnimator.start();
    } else {
      scheduleAnimator(scaleAnimator);
  ...
  void notifyOnMapClickListeners(@NonNull PointF tapPoint) {
    for (VietmapMap.OnMapClickListener listener : onMapClickListenerList) {
  ...
  void notifyOnMapLongClickListeners(@NonNull PointF longClickPoint) {
    for (VietmapMap.OnMapLongClickListener listener : onMapLongClickListenerList) {
  ...
  void notifyOnFlingListeners() {
    for (VietmapMap.OnFlingListener listener : onFlingListenerList) {
      listener.onFling();
    }
  }

  void notifyOnMoveBeginListeners(@NonNull MoveGestureDetector detector) {
    for (VietmapMap.OnMoveListener listener : onMoveListenerList) {
  ...
  void notifyOnRotateBeginListeners(@NonNull RotateGestureDetector detector) {
    for (VietmapMap.OnRotateListener listener : onRotateListenerList) {
  ...
  void notifyOnScaleBeginListeners(@NonNull StandardScaleGestureDetector detector) {
    for (VietmapMap.OnScaleListener listener : onScaleListenerList) {
  ...
  void notifyOnShoveBeginListeners(@NonNull ShoveGestureDetector detector) {
    for (VietmapMap.OnShoveListener listener : onShoveListenerList) {
  ...
  void addOnMapClickListener(VietmapMap.OnMapClickListener onMapClickListener) {
    onMapClickListenerList.add(onMapClickListener);
  }

  void removeOnMapClickListener(VietmapMap.OnMapClickListener onMapClickListener) {
    onMapClickListenerList.remove(onMapClickListener);
  }

  void addOnMapLongClickListener(VietmapMap.OnMapLongClickListener onMapLongClickListener) {
    onMapLongClickListenerList.add(onMapLongClickListener);
  }

  void removeOnMapLongClickListener(VietmapMap.OnMapLongClickListener onMapLongClickListener) {
    onMapLongClickListenerList.remove(onMapLongClickListener);
  }

  void addOnFlingListener(VietmapMap.OnFlingListener onFlingListener) {
    onFlingListenerList.add(onFlingListener);
  }

  void removeOnFlingListener(VietmapMap.OnFlingListener onFlingListener) {
    onFlingListenerList.remove(onFlingListener);
  }

  void addOnMoveListener(VietmapMap.OnMoveListener listener) {
    onMoveListenerList.add(listener);
  }

  void removeOnMoveListener(VietmapMap.OnMoveListener listener) {
    onMoveListenerList.remove(listener);
  }

  void addOnRotateListener(VietmapMap.OnRotateListener listener) {
    onRotateListenerList.add(listener);
  }

  void removeOnRotateListener(VietmapMap.OnRotateListener listener) {
    onRotateListenerList.remove(listener);
  }

  void addOnScaleListener(VietmapMap.OnScaleListener listener) {
    onScaleListenerList.add(listener);
  }

  void removeOnScaleListener(VietmapMap.OnScaleListener listener) {
    onScaleListenerList.remove(listener);
  }

  void addShoveListener(VietmapMap.OnShoveListener listener) {
    onShoveListenerList.add(listener);
  }

  void removeShoveListener(VietmapMap.OnShoveListener listener) {
