...
public final class CameraUpdateFactory {
  ...
  static final class CameraPositionUpdate implements CameraUpdate {
    ...
    @Override
    public CameraPosition getCameraPosition(@NonNull VietmapMap VietmapMap) {
      if (target == null) {
        CameraPosition previousPosition = VietmapMap.getCameraPosition();
  ...
  static final class CameraBoundsUpdate implements CameraUpdate {
    ...
    public CameraPosition getCameraPosition(@NonNull VietmapMap VietmapMap) {
      if (bearing == null && tilt == null) {
        ...
        return VietmapMap.getCameraForLatLngBounds(bounds, padding);
      } else {
        // use provided tilt and bearing
        assert bearing != null;
        assert tilt != null;
        return VietmapMap.getCameraForLatLngBounds(bounds, padding, bearing, tilt);
      }
    }
  ...
  static final class CameraMoveUpdate implements CameraUpdate {
    ...
    @Override
    public CameraPosition getCameraPosition(@NonNull VietmapMap VietmapMap) {
      UiSettings uiSettings = VietmapMap.getUiSettings();
      Projection projection = VietmapMap.getProjection();
      // Calculate the new center point
      float viewPortWidth = uiSettings.getWidth();
      float viewPortHeight = uiSettings.getHeight();
      int[] padding = VietmapMap.getPadding();
  ...
  static final class ZoomUpdate implements CameraUpdate {
    ...
    @Override
    public CameraPosition getCameraPosition(@NonNull VietmapMap VietmapMap) {
      CameraPosition cameraPosition = VietmapMap.getCameraPosition();
      if (getType() != CameraUpdateFactory.ZoomUpdate.ZOOM_TO_POINT) {
        ...
        return new CameraPosition.Builder(cameraPosition)
          .zoom(transformZoom(cameraPosition.zoom))
          .target(VietmapMap.getProjection().fromScreenLocation(new PointF(getX(), getY())))
          .build();
      }
    }
