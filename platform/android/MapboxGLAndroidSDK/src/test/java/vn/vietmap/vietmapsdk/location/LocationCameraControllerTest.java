...
import static vn.vietmap.vietmapsdk.location.MapboxAnimator.ANIMATOR_CAMERA_COMPASS_BEARING;
import static vn.vietmap.vietmapsdk.location.MapboxAnimator.ANIMATOR_CAMERA_GPS_BEARING;
import static vn.vietmap.vietmapsdk.location.MapboxAnimator.ANIMATOR_CAMERA_LATLNG;
import static vn.vietmap.vietmapsdk.location.MapboxAnimator.ANIMATOR_TILT;
import static vn.vietmap.vietmapsdk.location.MapboxAnimator.ANIMATOR_ZOOM;
import static vn.vietmap.vietmapsdk.location.modes.CameraMode.NONE;
import static vn.vietmap.vietmapsdk.location.modes.CameraMode.NONE_COMPASS;
import static vn.vietmap.vietmapsdk.location.modes.CameraMode.NONE_GPS;
import static vn.vietmap.vietmapsdk.location.modes.CameraMode.TRACKING;
import static vn.vietmap.vietmapsdk.location.modes.CameraMode.TRACKING_COMPASS;
import static vn.vietmap.vietmapsdk.location.modes.CameraMode.TRACKING_GPS;
import static vn.vietmap.vietmapsdk.location.modes.CameraMode.TRACKING_GPS_NORTH;
...
public class LocationCameraControllerTest {
  ...
  public void setCameraMode_notTrackingAdjustsFocalPoint() {
    VietmapMap VietmapMap = mock(VietmapMap.class);
    when(VietmapMap.getUiSettings()).thenReturn(mock(UiSettings.class));
    LocationCameraController camera = buildCamera(VietmapMap);
    camera.initializeOptions(mock(LocationComponentOptions.class));
    ...
    verify(VietmapMap.getUiSettings()).setFocalPoint(null);
  }
  ...
  public void setCameraMode_cancelTransitionsWhenSet() {
    VietmapMap VietmapMap = mock(VietmapMap.class);
    when(VietmapMap.getUiSettings()).thenReturn(mock(UiSettings.class));
    when(VietmapMap.getProjection()).thenReturn(mock(Projection.class));
    LocationCameraController camera = buildCamera(VietmapMap);
    camera.initializeOptions(mock(LocationComponentOptions.class));

    camera.setCameraMode(NONE_COMPASS);
    verify(VietmapMap, times(1)).cancelTransitions();

    camera.setCameraMode(NONE_GPS);
    verify(VietmapMap, times(2)).cancelTransitions();

    camera.setCameraMode(TRACKING);
    verify(VietmapMap, times(3)).cancelTransitions();

    camera.setCameraMode(TRACKING_COMPASS);
    verify(VietmapMap, times(4)).cancelTransitions();

    camera.setCameraMode(TRACKING_GPS);
    verify(VietmapMap, times(5)).cancelTransitions();

    camera.setCameraMode(TRACKING_GPS_NORTH);
    verify(VietmapMap, times(6)).cancelTransitions();
  }
  ...
  public void setCameraMode_dontCancelTransitionsWhenNoneSet() {
    ...
    camera.setCameraMode(NONE);
    verify(VietmapMap, never()).cancelTransitions();
  }
  ...
  public void onNewLatLngValue_cameraModeTrackingUpdatesLatLng() {
    ...
    verify(transform).moveCamera(any(VietmapMap.class), any(CameraUpdate.class),
      nullable(VietmapMap.CancelableCallback.class));
  }
  ...
  public void onNewLatLngValue_cameraModeNoneIgnored() {
    ...
    verify(transform, times(0)).moveCamera(any(VietmapMap.class), any(CameraUpdate.class),
      nullable(VietmapMap.CancelableCallback.class));
  }
  ...
  public void onNewLatLngValue_focalPointIsAdjusted() {
    ...
    final VietmapMap.OnCameraMoveListener[] listener = {null};
    doAnswer(new Answer<Void>() {
      @Override
      public Void answer(InvocationOnMock invocation) {
        listener[0] = (VietmapMap.OnCameraMoveListener) invocation.getArguments()[0];
        return null;
      }
    }).when(VietmapMap).addOnCameraMoveListener(any(VietmapMap.OnCameraMoveListener.class));

    Projection projection = mock(Projection.class);
    PointF pointF = mock(PointF.class);
    when(projection.toScreenLocation(any(LatLng.class))).thenReturn(pointF);
    when(VietmapMap.getProjection()).thenReturn(projection);
    LocationCameraController camera = buildCamera(VietmapMap, transform);
    LocationComponentOptions options = mock(LocationComponentOptions.class);
    when(options.trackingGesturesManagement()).thenReturn(true);
    camera.initializeOptions(options);
    camera.setCameraMode(TRACKING);
    ...
    when(VietmapMap.getUiSettings()).thenReturn(uiSettings);
  ...
  public void onNewGpsBearingValue_cameraModeTrackingNorthUpdatesBearing() {
    ...
    when(VietmapMap.getCameraPosition()).thenReturn(cameraPosition);
    camera.initializeOptions(mock(LocationComponentOptions.class));
    camera.setCameraMode(TRACKING_GPS_NORTH);
    float gpsBearing = 5f;
    ...
    verify(transform).moveCamera(eq(VietmapMap), any(CameraUpdate.class),
      nullable(VietmapMap.CancelableCallback.class));
  }
  ...
  public void onNewGpsBearingValue_cameraModeTrackingNorthBearingZeroIgnored() {
    ...
    verify(transform, times(0)).moveCamera(eq(VietmapMap), any(CameraUpdate.class),
      nullable(VietmapMap.CancelableCallback.class));
  }
