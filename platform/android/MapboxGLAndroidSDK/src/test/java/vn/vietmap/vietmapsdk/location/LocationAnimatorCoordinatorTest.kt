...
import vn.vietmap.vietmapsdk.location.LocationComponentConstants.DEFAULT_TRACKING_TILT_ANIM_DURATION
import vn.vietmap.vietmapsdk.location.LocationComponentConstants.DEFAULT_TRACKING_ZOOM_ANIM_DURATION
import vn.vietmap.vietmapsdk.location.MapboxAnimator.*
...
class LocationAnimatorCoordinatorTest {
    ...
    private val cameraPosition: vn.vietmap.vietmapsdk.camera.CameraPosition = vn.vietmap.vietmapsdk.camera.CameraPosition.DEFAULT

    private val animatorProvider: vn.vietmap.vietmapsdk.location.MapboxAnimatorProvider = mockk()
    private val animatorSetProvider: vn.vietmap.vietmapsdk.location.MapboxAnimatorSetProvider = mockk()

    private val projection: vn.vietmap.vietmapsdk.maps.Projection = mockk()

    @Before
    fun setUp() {
        locationAnimatorCoordinator = vn.vietmap.vietmapsdk.location.LocationAnimatorCoordinator(
            projection,
            animatorSetProvider,
            animatorProvider
        )
    ...
    private fun configureAnimatorProvider() {
        ...
        registerInstanceFactory { AnimationsValueChangeListener<vn.vietmap.vietmapsdk.geometry.LatLng> {} }
        val floatsSlot = slot<Array<Float>>()
        val listenerSlot = slot<vn.vietmap.vietmapsdk.location.MapboxAnimator.AnimationsValueChangeListener<*>>()
        val maxFpsSlot = slot<Int>()
        every {
            animatorProvider.floatAnimator(capture(floatsSlot), capture(listenerSlot), capture(maxFpsSlot))
        } answers {
            vn.vietmap.vietmapsdk.location.MapboxFloatAnimator(
                floatsSlot.captured,
        ...
        val latLngsSlot = slot<Array<vn.vietmap.vietmapsdk.geometry.LatLng>>()
        every {
            animatorProvider.latLngAnimator(capture(latLngsSlot), capture(listenerSlot), capture(maxFpsSlot))
        } answers {
            vn.vietmap.vietmapsdk.location.MapboxLatLngAnimator(
        ...
        val callback = slot<vn.vietmap.vietmapsdk.maps.VietmapMap.CancelableCallback>()
        every {
            animatorProvider.cameraAnimator(capture(floatsSlot), capture(listenerSlot), capture(callback))
        } answers {
            vn.vietmap.vietmapsdk.location.MapboxCameraAnimatorAdapter(
                floatsSlot.captured,
                listenerSlot.captured,
                callback.captured
            )
    ...
    fun feedNewLocation_animatorValue() {
        ...
        val cameraLatLngTarget = locationAnimatorCoordinator.animatorArray[ANIMATOR_CAMERA_LATLNG]?.target as vn.vietmap.vietmapsdk.geometry.LatLng
        assertEquals(location.latitude, cameraLatLngTarget.latitude)
        assertEquals(location.longitude, cameraLatLngTarget.longitude)

        val layerLatLngTarget = locationAnimatorCoordinator.animatorArray[ANIMATOR_LAYER_LATLNG]?.target as vn.vietmap.vietmapsdk.geometry.LatLng
        ...
        verify {
            animatorProvider.latLngAnimator(
                arrayOf(
                    ...
                    vn.vietmap.vietmapsdk.geometry.LatLng(
                        locationInter.latitude,
                        locationInter.longitude
                    ),
                    vn.vietmap.vietmapsdk.geometry.LatLng(location.latitude, location.longitude)
                ),
                any(),
                any()
            )
    ...
    fun feedNewLocation_animatorValue_correctRotation_1() {
        ...
        val animator = mockk<vn.vietmap.vietmapsdk.location.MapboxFloatAnimator>(relaxed = true)
    ...
    fun addNewListener() {
        ...
        val holder =
            vn.vietmap.vietmapsdk.location.AnimatorListenerHolder(
                vn.vietmap.vietmapsdk.location.modes.RenderMode.NORMAL,
                listener
            )
        val set = HashSet<vn.vietmap.vietmapsdk.location.AnimatorListenerHolder>().also {
            it.add(holder)
        }
    ...
    fun updateListeners() {
        val listener = Mockito.mock(AnimationsValueChangeListener::class.java)
        val holder =
            vn.vietmap.vietmapsdk.location.AnimatorListenerHolder(ANIMATOR_LAYER_LATLNG, listener)
        val set = HashSet<vn.vietmap.vietmapsdk.location.AnimatorListenerHolder>().also {
            it.add(holder)
        }
        locationAnimatorCoordinator.updateAnimatorListenerHolders(set)
        ...
        val holder2 =
            vn.vietmap.vietmapsdk.location.AnimatorListenerHolder(ANIMATOR_LAYER_LATLNG, listener2)
        val listener3 = Mockito.mock(AnimationsValueChangeListener::class.java)
        val holder3 = vn.vietmap.vietmapsdk.location.AnimatorListenerHolder(
            ANIMATOR_LAYER_ACCURACY,
            listener3
        )
        val set2 = HashSet<vn.vietmap.vietmapsdk.location.AnimatorListenerHolder>().also {
            it.add(holder2)
            it.add(holder3)
        }
    ...
    fun updateListeners_listenerRemoved_animtorInvalid() {
        ...
        val holder = vn.vietmap.vietmapsdk.location.AnimatorListenerHolder(
            ANIMATOR_LAYER_ACCURACY,
            listener
        )
        val set = HashSet<vn.vietmap.vietmapsdk.location.AnimatorListenerHolder>().also {
    ...
    fun feedNewCompassBearing_withAnimation() {
        ...
        verify(exactly = 1) { animatorSetProvider.startAnimation(eq(animators), ofType(LinearInterpolator::class), eq(
            vn.vietmap.vietmapsdk.location.LocationComponentConstants.COMPASS_UPDATE_RATE_MS)) }
    }
    ...
    fun feedNewAccuracy_withAnimation() {
        ...
        verify(exactly = 1) { animatorSetProvider.startAnimation(eq(animators), ofType(LinearInterpolator::class), eq(
            vn.vietmap.vietmapsdk.location.LocationComponentConstants.ACCURACY_RADIUS_ANIMATION_DURATION)) }
    }
    ...
    private fun getListenerHoldersSet(vararg animatorTypes: Int): Set<vn.vietmap.vietmapsdk.location.AnimatorListenerHolder> {
        return HashSet<vn.vietmap.vietmapsdk.location.AnimatorListenerHolder>().also {
